...
export default function DropDown(props) {
  ...
  const rootStyle = useMemo(() => {
    return position !== POSITION.STATIC ? { width: '100%', height: dimension.height } : undefined;
  }, [dimension.height, position]);
  ...
  useEffect(() => {
    positionTimeoutHandle.current = setTimeout(() => {
      const { width, height } = dropDownNode.current.getBoundingClientRect();
  ...
  return (
    <div data-react-toolbox="menu" className={dropDownClassName} style={rootStyle} ref={rootRef}>
      {outline ? <div className={styles.outline} style={outlineStyle} /> : null}
      <ul ref={dropDownNode} className={styles.menuInner} style={dropDownStyle}>
